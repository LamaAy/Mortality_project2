...
/* Compact workflow display: show only agent name, run controls, and final output. */
.agent-checklist,
.agent-prompt,
.agent-condition,
.agent-mini-stepper {
...
LAY_QUERY_EXPANSIONS = {
    ...
    "cardiac arrest": ["cardiac arrest", "I46.9"],
    "respiratory failure": ["respiratory failure", "J96.9"],
    "acute respiratory failure": ["acute respiratory failure", "J96.0"],
    "ingrown toenail": ["ingrowing nail", "L60.0"],
}
...
def candidate_adjustment_score(row: pd.Series, query: str, sex_value: str, role: str) -> Tuple[float, List[str]]:
    ...
    if "cardiac arrest" in q or "heart stopped" in q or "cardiorespiratory arrest" in q:
        if code_norm.startswith("I469") or code == "I46.9":
            score += 8.0
            reasons.append("preferred cardiac arrest I46.9 for SP7 detection")
        if code_norm.startswith("I46"):
            score += 4.0

    if "respiratory failure" in q:
        if code_norm.startswith("J96"):
            score += 8.0
            reasons.append("preferred respiratory failure J96 family for SP7 detection")

    if "ingrown toenail" in q or "ingrowing nail" in q:
        if code_norm.startswith("L600") or code == "L60.0":
            score += 8.0
            reasons.append("preferred ingrowing nail L60.0 for SP8 detection")
        if code_norm.startswith(("S", "T")):
            score -= 5.0
            reasons.append("injury code penalized for ingrown toenail text")
...
def has_multiple_causes_in_one_line(cause_text: str) -> bool:
    """
    Detect only CLEAR multiple-cause entries in one certificate line.

    This function is intentionally conservative. It should not block ICD-style
    diagnostic phrases that contain words such as "with", "without",
    "following", "due to", or a single comma. Those phrases may be one
    coded diagnosis, for example:
      - Aphasia following nontraumatic intracerebral hemorrhage
      - Acute respiratory failure, unsp w hypoxia or hypercapnia
      - Type 2 diabetes mellitus without complications

    It should block only obvious lists of separate conditions placed on one line.
    """
    c = normalize_text_basic(cause_text)
    if not c:
        return False

    # Strong separators usually mean several separate conditions were placed
    # into one field. These should be split by the doctor.
    strong_separators = [";", " plus ", " along with "]
    if any(sep in c for sep in strong_separators):
        return True

    # Slash is ambiguous. Block only when both sides look like substantial
    # disease phrases, not shorthand or coding-style wording.
    if "/" in c:
        parts = [p.strip() for p in c.split("/") if p.strip()]
        if len(parts) >= 2 and all(len(tokenize(p)) >= 2 for p in parts):
            return True

    # Commas are common inside ICD descriptions, e.g. "respiratory failure,
    # unspecified...". Do not block one comma. Block only obvious long lists.
    comma_parts = [p.strip() for p in c.split(",") if p.strip()]
    if len(comma_parts) >= 3:
        return True

    # Allow common ICD-style connectors. They describe a single diagnosis often
    # enough that Structure Check should not block them. Later coding/sequence
    # checks can still warn if the causal chain is clinically implausible.
    soft_icd_connectors = [
        " with ",
        " without ",
        " following ",
    ...
    if any(conn in c for conn in soft_icd_connectors):
        return False

    # "and" may indicate a list, but it may also be part of a named ICD
    # phrase. Block only when there are many coordinated phrases or both sides
    # look like independent substantial diagnoses.
    and_split = re.split(r"\s+\band\b\s+", c)
    if len(and_split) >= 3:
        return True

    if len(and_split) == 2:
        left, right = and_split[0].strip(), and_split[1].strip()
        single_phrase_allow = [
            "signs and symptoms",
            "nausea and vomiting",
        ...
        if any(p in c for p in single_phrase_allow):
            return False
        if len(tokenize(left)) >= 2 and len(tokenize(right)) >= 2:
            return True
...
def pre_validate_structured_cod(part1_chain: List[Dict], part2_conditions: List[Dict]) -> Dict:
    ...
    for label, x in all_lines:
        ...
        if has_multiple_causes_in_one_line(cause):
            # Doctor-facing guardrail: block only clear lists of separate
            # conditions in one field. Do not block ICD-style diagnostic
            # phrases containing words such as "with", "without",
            # "following", or a single comma.
            issues.append({
                "severity": "error",
                "line": label,
                "type": "multiple_separate_causes_one_line",
                "sp_rule": "SP2",
                "message": (
                    "Several separate conditions appear to be listed on one line. "
                    "Please enter one condition per line in the causal chain."
                ),
                "blocking": True,
            })
...
with st.sidebar:
    ...
    with st.expander("Admin / data status", expanded=False):
        ...
        who_client_ok = bool(os.environ.get("WHO_ICD_CLIENT_ID") and os.environ.get("WHO_ICD_CLIENT_SECRET"))
        try:
            who_client_ok = who_client_ok or bool(st.secrets.get("WHO_ICD_CLIENT_ID") and st.secrets.get("WHO_ICD_CLIENT_SECRET"))
        except Exception:
            pass
        if who_client_ok:
            st.success("WHO ICD API credentials configured")
        else:
            st.info("WHO ICD API not configured; local ICD file will be used only")
...
You are Structure Check: Structure Check for an electronic death certificate.
...
You are ICD Coding: ICD Coding for an electronic death certificate.
...
You are Table A/B Rule Trace: Table A/B Rule Trace for an electronic death certificate.
...
def render_agent_stepper(current_step: int) -> None:
    ...
    for num, label in labels:
        ...
        html_pills.append(f'<span class="{cls}">{escape(label)}</span>')
        if num < 3:
...
def render_agent2_result(result: Dict, coded_results: Optional[Dict] = None) -> None:
    ...
    if not result:
        st.markdown(
            ...
            '<div class="agent-hidden-details-note">Run ICD Coding to retrieve and select ICD codes for all Part I and Part II entries.</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        return
    ...
    for item in coded_causes:
        ...
        if who_status_raw == "verified":
            who_status = "WHO verified"
        elif who_status_raw == "parent_verified":
            who_status = "WHO parent verified"
        elif who_status_raw == "not_configured":
            who_status = "WHO API not configured"
        elif who_status_raw == "failed":
            who_status = "WHO verification failed"
        else:
            who_status = "WHO not checked"
...
def render_agent3_result(result: Dict) -> None:
    ...
    if not result:
        st.markdown(
            '<div class="agent-output-box">'
            '<div class="agent-kicker">TABLE A/B RULE TRACE</div>'
            '<div class="agent-title">Table A/B Rule Trace</div>'
            '<div class="agent-hidden-details-note">Run the Table A/B Rule Trace to see the SP rule, selected line, and UCOD decision.</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        return
    ...
    html_out = (
        ...
        f'<div class="agent-title">Table A/B Rule Trace</div>'
        f'<div class="agent-output-status {status_class}">Output: {escape(status_label)}</div>'
        f'<div>{escape(summary)}</div>'
...
def render_doctor_edit_panel(fd: Dict) -> Tuple[List[Dict], List[Dict]]:
    """Left-side editable doctor panel used on the review page."""
    ...
    # Doctor-facing UI is direct: no extra green action buttons in the editable panel.
    # The current field values are saved automatically and reused when the review checks run.
...
# =============================================================================
# NEW RULE-GROUNDED OVERRIDES
# Local ICD search + optional WHO ICD API verification + deterministic Table A/B SP logic
# =============================================================================

def clinical_step_label(step: int) -> str:
    return {
        1: "Structure Check",
        2: "ICD Coding",
        3: "Table A/B Rule Trace",
    }.get(int(step or 0), "Review Step")


def code_norm_for_rules(code: str) -> str:
    """Internal no-dot ICD code used for Table A/B matching."""
    return re.sub(r"[^A-Z0-9]", "", str(code or "").upper().strip())


def dot_code_from_norm(code: str) -> str:
    """Convert common ICD-10 no-dot diagnosis code to dotted display form."""
    c = code_norm_for_rules(code)
    if len(c) > 3:
        return c[:3] + "." + c[3:]
    return c


def parent_code_for_who(code: str) -> str:
    """Parent ICD-10 category, e.g., I50.9 -> I50."""
    c = code_norm_for_rules(code)
    return c[:3] if len(c) >= 3 else c


def get_who_api_credentials() -> Tuple[str, str]:
    client_id = os.environ.get("WHO_ICD_CLIENT_ID", "")
    client_secret = os.environ.get("WHO_ICD_CLIENT_SECRET", "")
    try:
        client_id = client_id or str(st.secrets.get("WHO_ICD_CLIENT_ID", ""))
        client_secret = client_secret or str(st.secrets.get("WHO_ICD_CLIENT_SECRET", ""))
    except Exception:
        pass
...
def who_icd_get_token_cached(client_id: str, client_secret: str) -> Dict:
    """Get WHO ICD API OAuth token. Cached slightly under the normal one-hour lifetime."""
    import requests
    token_endpoint = "https://icdaccessmanagement.who.int/connect/token"
    payload = {"grant_type": "client_credentials", "scope": "icdapi_access"}
    r = requests.post(token_endpoint, data=payload, auth=(client_id, client_secret), timeout=20)
    r.raise_for_status()
    return r.json()


@st.cache_data(ttl=86400, show_spinner=False)
def who_icd10_lookup_cached(client_id: str, client_secret: str, code: str, release_id: str = "2019") -> Dict:
    """Verify an ICD-10 code/category with WHO ICD API. Used after local ICD selection."""
    import requests
    token_data = who_icd_get_token_cached(client_id, client_secret)
    ...
    url = f"https://id.who.int/icd/release/10/{release_id}/{code}"
...
def verify_code_with_who_api(selected_code: str, release_id: str = "2019") -> Dict:
    """
    Verify selected local ICD code with WHO ICD API when credentials exist.
    The app keeps the selected local detailed code even if only the parent category verifies.
    """
    selected_code = str(selected_code or "").strip()
    if not selected_code:
    ...
    attempts = []
    for a in [selected_code.strip().upper(), dot_code_from_norm(norm), norm, parent_code_for_who(norm)]:
        if a and a not in attempts:
            attempts.append(a)

    last_error = ""
    for attempt in attempts:
        try:
            info = who_icd10_lookup_cached(client_id, client_secret, attempt, release_id=release_id)
            verified_parent = (code_norm_for_rules(attempt) == parent_code_for_who(norm) and code_norm_for_rules(attempt) != norm)
            return {
                "status": "parent_verified" if verified_parent else "verified",
                "selected_code": selected_code,
                "verified_code": attempt,
                "release_id": release_id,
                "message": f"WHO ICD API verified {'parent category ' if verified_parent else ''}{attempt}.",
                "who_info": info,
            }
        except Exception as e:
    ...
    return {
        ...
        "release_id": release_id,
        "message": "WHO ICD API lookup failed; local ICD file remains the coding source for this prototype.",
        "error": last_error,
    }


def attach_who_verification_to_results(coded_results: Dict, release_id: str = "2019") -> Dict:
    """Attach optional WHO API verification to each selected ICD code in the coding result."""
...
def query_taba_pair(taba_df: pd.DataFrame, upper_item: Dict, lower_item: Dict) -> Dict:
    """One Table A check: upper/effect is the address; lower/cause is searched underneath."""
    upper_code = str(upper_item.get("code_formatted", "") or "").strip()
    lower_code = str(lower_item.get("code_formatted", "") or "").strip()
    result = query_taba(taba_df=taba_df, upper_code=upper_code, lower_code=lower_code)
    return {
        "upper_line": upper_item.get("line", ""),
        "upper_cause": upper_item.get("cause", ""),
        "upper_code": upper_code,
        "lower_line": lower_item.get("line", ""),
        "lower_cause": lower_item.get("cause", ""),
        "lower_code": lower_code,
        "accepted": result.get("accepted"),
        "reason": result.get("reason", ""),
        "matches": result.get("matches", [])[:3],
...
def check_part1_sequence_with_taba(part1_items: List[Dict], taba_df: pd.DataFrame) -> Dict:
    """
    Table A sequence check following the doctor's rule:
    - SP3: test the lowest used Part I line against every line above it.
    - If SP3 fails, also compute adjacent links to support SP4/SP5 trace.
    """
    filled = [x for x in part1_items if str(x.get("code_formatted", "") or "").strip()]
    available = bool(taba_df is not None and not taba_df.empty)
    ...
    if len(filled) <= 1:
        return {
            ...
            "sp3_bottom_checks": [],
            "adjacent_links": [],
            "summary": "Single Part I ICD-coded cause; no Table A sequence check is needed.",
        }

    bottom = filled[-1]
    bottom_checks = [query_taba_pair(taba_df, upper, bottom) for upper in filled[:-1]]
    uncertain = any(x.get("accepted") is None for x in bottom_checks)
    sp3_valid = bool(bottom_checks) and all(x.get("accepted") is True for x in bottom_checks)

    adjacent_links = [query_taba_pair(taba_df, upper, lower) for upper, lower in zip(filled, filled[1:])]

    if not available:
        summary = "Table A was not loaded; causal sequence cannot be table-confirmed."
    elif sp3_valid:
        summary = "Table A confirms SP3: the lowest used Part I line explains all lines above it."
    elif uncertain:
        summary = "Table A sequence check is uncertain because one or more ICD links could not be checked."
    else:
        summary = "Table A does not confirm SP3; SP4/SP5 review is required."

    return {
        "available": available,
        "valid_sequence": sp3_valid,
        "uncertain": uncertain,
        "links": bottom_checks,
        "sp3_bottom_checks": bottom_checks,
        "adjacent_links": adjacent_links,
        "summary": summary,
    }


def apply_sp1_to_sp5_with_taba(coded_causes: List[Dict], taba_df: pd.DataFrame) -> Dict:
    """Deterministic SP1-SP5 selection. SP3-SP5 use Table A only."""
    part1 = [x for x in coded_causes or [] if x.get("role") in {"immediate", "contributing", "underlying"}]
    part2 = [x for x in coded_causes or [] if x.get("role") == "other"]
    conditions = part1 + part2

    if not conditions:
        return {
            "sp_rule": "REVIEW", "selected_line": "", "selected_cause": "", "selected_code": "",
            "full_sequence_valid": False, "partial_sequence_valid": False, "causal_links": [],
            "warnings": ["No condition was coded."], "needs_manual_review": True,
            "explanation": "No coded cause of death is available.",
        }
    ...
    if not part1:
        x = conditions[0]
        return {
            "sp_rule": "SP5", "selected_line": x.get("line", ""), "selected_cause": x.get("cause", ""),
            "selected_code": x.get("code_formatted", ""), "full_sequence_valid": False,
            "partial_sequence_valid": False, "causal_links": [],
            "warnings": ["Only Part II conditions are present; coder review is required."], "needs_manual_review": True,
            "explanation": "SP5 used as fallback because there is no usable Part I sequence.",
        }

    taba_seq = check_part1_sequence_with_taba(part1, taba_df)
    bottom_checks = taba_seq.get("sp3_bottom_checks", []) or []
    ...
    # SP4: find the deepest adjacent valid run that still reaches Part I(a).
    adjacent = taba_seq.get("adjacent_links", []) or []
    deepest_index = 0
    for idx, link in enumerate(adjacent):
        if link.get("accepted") is True:
            deepest_index = idx + 1
        else:
            break
...
def sp7_is_hard_ill_defined(item: Dict) -> bool:
    """SP7: hard-stop ill-defined / terminal mechanism checks.

    Use both the selected ICD code and doctor text because either the ICD mapping
    or the Excel metadata can be incomplete. This check is applied only to the
    final selected starting point after SP6.
    """
    if not item:
        return False

    code_norm = code_norm_for_rules(
    ...
    # Text phrases that describe mode/mechanism of dying, not the disease that
    # started the fatal chain.
    hard_terms = [
        "cardiac arrest", "heart stopped", "cardiorespiratory arrest",
        "respiratory arrest", "respiratory failure", "acute respiratory failure",
        "multi organ failure", "multi-organ failure", "multiple organ failure",
        "organ failure", "circulatory failure", "hypotension", "collapse",
        "sudden cardiac death", "sudden death", "old age", "senility",
        "unknown cause", "unknown", "natural causes", "unascertained",
    ]
    if any(t in cause for t in hard_terms):
        return True

    # Excel/local flags remain authoritative when available.
    if "ill-defined" in text_flags or "ill defined" in text_flags or "illdefined" in text_flags:
        return True

    # ICD fallback list for common ill-defined/terminal mechanisms.
    # R-codes are symptoms/signs/ill-defined conditions; I46 is cardiac arrest;
    # J96 is respiratory failure; I95 covers hypotension.
    if code_norm.startswith("R"):
        return True
    if code_norm.startswith(("I46", "J96", "I95")):
        return True
    return False


def sp8_is_unacceptable_or_unlikely(item: Dict, coded_causes: List[Dict]) -> bool:
    """SP8: hard-stop unlikely/trivial/unacceptable UCOD checks.

    This combines Excel metadata, explicit fallback code/text lists, and the
    injury-without-external-cause guardrail.
    """
    if not item:
        return False
    ...
    # Local metadata flags.
    if is_excel_unlikely_to_cause_death(item):
        return True
    if acceptable_main_bool(item.get("acceptable_main", "")) is False:
        return True

    # Common trivial/minor conditions that should not be accepted as UCOD without
    # a fatal complication explicitly documented.
    trivial_terms = [
        "ingrown toenail", "in-grown toenail", "toenail", "nail disorder",
        "acne", "corn", "corns", "callus", "callosities", "wart",
        "skin tag", "minor abrasion", "superficial injury", "sprain",
    ]
    if any(t in cause for t in trivial_terms):
        return True

    # ICD fallback examples: L60.0 ingrowing nail; L70 acne; L84 corns/callosities;
    # B07 viral warts. These are unlikely to be UCOD unless a real fatal chain is added.
    if code_norm.startswith(("L600", "L70", "L84", "B07")):
        return True

    # Nature of injury without external cause mechanism is not acceptable as final UCOD.
    if _injury_without_external_cause(item, coded_causes):
        return True
...
def apply_sp6_with_tabb(sp_review: Dict, coded_causes: List[Dict], tabb_df: pd.DataFrame) -> Dict:
    ...
    out.setdefault("warnings", [])
    trace = []
    if tabb_df is None or tabb_df.empty or not coded_causes:
        out["sp6_trace"] = trace
        return out

    current_code = out.get("selected_code") or selected_code_for_sp(out, coded_causes)
    current_line = str(out.get("selected_line", ""))
    current_cause = str(out.get("selected_cause", ""))

    for _ in range(5):
        changed = False
        for other in coded_causes:
            other_code = other.get("code_formatted", "")
            if not other_code or code_norm_for_rules(other_code) == code_norm_for_rules(current_code):
                continue
            ...
            if ds_matches:
                trace.append({
                    "from_tsp_code": current_code,
                    "from_tsp_cause": current_cause,
                    "to_obvious_cause_code": other_code,
                    "to_obvious_cause": other.get("cause", ""),
                    "line": other.get("line", ""),
                    "matches": ds_matches[:3],
                })
                current_code = other_code
                current_line = str(other.get("line", ""))
                current_cause = str(other.get("cause", ""))
                changed = True
                break
        if not changed:
            break
    ...
    if trace:
        ...
        out["selected_cause"] = current_cause
        out["needs_manual_review"] = True
        out["warnings"].append("SP6 Table B direct-sequel rule changed the tentative starting point; coder review is recommended.")
        out["explanation"] = "SP6 applied: Table B found an obvious cause elsewhere on the certificate and shifted the tentative starting point."
    out["sp6_trace"] = trace
    return out


def apply_sp7_sp8_quality(sp_review: Dict, coded_causes: List[Dict]) -> Dict:
    ...
    for item in coded_causes or []:
        if str(item.get("line", "")).lower() == selected_line or code_norm_for_rules(item.get("code_formatted", "")) == selected_code:
            selected = item
            break
    if not selected:
        return out
    ...
    sp8 = sp8_is_unacceptable_or_unlikely(selected, coded_causes)
    out["sp7_sp8_quality"] = {
        ...
        "doctor_action_required": bool(sp7 or sp8),
    }
    if sp7:
        msg = (
    ...
    if sp8:
        if injury_missing_external:
            ...
            out["warnings"].append(
                "SP8 hard stop: the selected starting point appears unlikely/trivial or not acceptable as the underlying cause of death. Return to Structure Check and enter a more plausible disease, injury mechanism, or causal sequence."
            )
            out["blocking"] = True
            out["doctor_action"] = "Go back and revise the selected cause or add the missing fatal causal condition."
        out["needs_manual_review"] = True
...
def apply_sp_engine(api_key: str, concepts: Dict, coded_causes: List[Dict]) -> Dict:
    """Rule-grounded SP engine. LLM is not used for SP3-SP8 decisions."""
    taba_df = load_taba_rules()
    tabb_df = load_tabb_rules()
    sp = apply_sp1_to_sp5_with_taba(coded_causes, taba_df)
    sp = apply_sp6_with_tabb(sp, coded_causes, tabb_df)
    sp = apply_sp7_sp8_quality(sp, coded_causes)
    sp["selected_code"] = sp.get("selected_code") or selected_code_for_sp(sp, coded_causes)
    return sp
...
def agent3_mortality_sequence_with_llm(api_key: str, coded_results: Dict, tabb_df: pd.DataFrame, taba_df: Optional[pd.DataFrame] = None) -> Dict:
    ...
    summary = f"{sp_review.get('sp_rule','REVIEW')} selected Part I ({line}) — {selected} ({code})." if selected else "Rule trace could not select a reliable UCOD starting point."

    rule_issues = []
    for w in sp_review.get("warnings", []) or []:
        rule_issues.append({"line": line or "Part I", "severity": "warning", "message": str(w)})
    if not taba_sequence.get("available"):
        rule_issues.append({"line": "Part I", "severity": "warning", "message": "Table A file was not loaded; SP3-SP5 sequence was not table-confirmed."})

    validation["taba_sequence"] = taba_sequence
    validation["tabb_result"] = tabb_result
    validation["sp_review"] = sp_review
    validation["agent3_sequence_status"] = status
    validation["agent3_sequence_summary"] = summary
    validation["overall_quality"] = "Excellent" if status == "pass" and not validation.get("coding_issues") else (validation.get("overall_quality") or "Needs Review")
...
def agent4_obvious_cause_with_tabb(coded_results: Dict, tabb_df: Optional[pd.DataFrame] = None) -> Dict:
    ...
    # SP7/SP8 are quality gates on the final selected starting point AFTER SP6.
    # This is where cardiac arrest/respiratory failure/unknown cause and trivial
    # causes such as ingrown toenail are blocked before the final certificate.
    final_sp = apply_sp7_sp8_quality(final_sp, coded_causes)
    blocked = bool(final_sp.get("blocking"))
    ...
    return {
        "status": "block" if blocked else ("warning" if sp6.get("changed") else "pass"),
        "summary": sp6.get("summary", "SP6 completed."),
        "blocking": blocked,
...
# =============================================================================
    ...
    if False:
        ...
        # Build a simple doctor-facing Good / Needs Attention preview for Structure Check only.
        ...
        if not attention_items:
            attention_items.append("No bad input/form issue detected by the Structure Check.")
        ...
        st.markdown(
            ...
            <div class="agent-preview-card">
              <div class="agent-kicker">STRUCTURE CHECK</div>
              <div class="agent-preview-title">Structure Check</div>
              ...
              <div class="agent-condition">
                ...
                ICD Coding opens only when the Structure Check has no blocking input/form errors.
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.caption("The yellow medical/ICD warnings under the fields are suggestions. Full ICD retrieval starts in ICD Coding.")

    # No extra action buttons here. The sidebar Navigation controls page movement.
    # Store the prepared review context silently so Review & Coding opens directly.
    if precheck["part1_chain"]:
        for x in precheck["part1_chain"]:
...
# =============================================================================
    ...
    def _render_rules_result_simple(result: Dict) -> None:
        # Doctor-facing SP3/SP4/SP5 sequence decision using Table A only.
        if not result:
            st.info("Click Check Sequence first.")
            return

        sp = result.get("sp_review", {}) or {}
        taba = result.get("taba_sequence") or sp.get("taba_sequence") or {}
        sp3_checks = taba.get("sp3_bottom_checks") or taba.get("links") or []
        adjacent = taba.get("adjacent_links") or []

        base_rule = str(sp.get("base_sp_rule") or sp.get("sp_rule") or "REVIEW")
        selected_line = str(sp.get("selected_line", "") or "")
        selected_cause = str(sp.get("selected_cause", "") or "")
        selected_code = str(sp.get("selected_code", "") or "")

        if base_rule == "SP3":
            accent = "#006940"; bg = "#ffffff"; border = "#d8e6dc"; icon = "●"; status = "Sequence accepted"
            why = "The lowest Part I condition explains every condition above it, so the bottom line is selected."
        elif base_rule == "SP4":
            accent = "#a66a00"; bg = "#fffaf0"; border = "#e7c46a"; icon = "⚠"; status = "Partial sequence accepted"
            why = "The bottom condition did not explain everything, but Table A found a valid causal run that still reaches line (a)."
        elif base_rule == "SP5":
            accent = "#c0392b"; bg = "#fff7f7"; border = "#e9b3b3"; icon = "✕"; status = "No acceptable chain found"
            why = "No acceptable Table A sequence reaches line (a), so the first-mentioned Part I condition is retained as the review candidate."
        else:
            accent = "#415a4d"; bg = "#ffffff"; border = "#d8e6dc"; icon = "⚠"; status = "Sequence reviewed"
            why = str(sp.get("explanation", "Sequence review completed."))

        selected_label = "Pending"
        if selected_cause:
            selected_label = f"Part I ({selected_line}) — {selected_cause} ({selected_code})"
        ...
        st.markdown(
            ...
            <div style="border:1px solid {border};border-radius:18px;padding:1rem 1rem;background:{bg};box-shadow:0 10px 28px rgba(0,0,0,.04);max-width:100%;overflow:hidden;margin-bottom:1rem;">
              <div style="font-size:1.15rem;font-weight:900;color:#10233f;margin-bottom:.35rem;">
                <span style="color:{accent};font-weight:900;margin-right:.35rem;">{icon}</span> Table A sequence check
              </div>
              <div style="display:grid;grid-template-columns:1fr;gap:.45rem;line-height:1.55;">
                <div><b>Applied rule:</b> <span style="color:{accent};font-weight:900;">{escape(base_rule)}</span></div>
                <div><b>Status:</b> {escape(status)}</div>
                <div><b>Selected starting point:</b> {escape(selected_label)}</div>
        ...
        def _accepted_text(val):
            if val is True:
                return "Yes", "✅ Found", "#006940"
            if val is False:
                return "No", "✕ Not found", "#c0392b"
            return "Unclear", "⚠ Review", "#a66a00"

        def _render_table(title: str, subtitle: str, rows: List[Dict], mode: str) -> None:
            if not rows:
                st.markdown(
                    f'''
                    <div style="border:1px solid #d8e6dc;border-radius:16px;padding:.85rem .95rem;background:#fff;margin-bottom:1rem;">
                      <div style="font-weight:900;color:#10233f;">{escape(title)}</div>
                      <div style="color:#5a7060;font-size:.86rem;margin-top:.25rem;">No Table A checks available.</div>
                    </div>
                    ''',
                    unsafe_allow_html=True,
                )
            ...
            for i, link in enumerate(rows, start=1):
                accepted = link.get("accepted")
                answer, result_text, color = _accepted_text(accepted)
                ...
                if mode == "sp3":
                    question = f"Does line ({lower_line}) explain line ({upper_line})?"
                else:
                    question = f"Does line ({lower_line}) cause line ({upper_line})?"
                pair = f"{lower_cause} → {upper_cause}"
                codes = f"Search: {lower_code}; Address: {upper_code}"
                html_rows.append(f'''
                  <tr>
                    <td style="padding:.52rem .45rem;border-bottom:1px solid #e5eee9;text-align:center;font-weight:900;vertical-align:top;">{i}</td>
                    <td style="padding:.52rem .45rem;border-bottom:1px solid #e5eee9;line-height:1.35;vertical-align:top;word-break:break-word;overflow-wrap:anywhere;">
                      <b>{escape(question)}</b><br>
                      <span style="font-size:.82rem;color:#293a31;">{escape(pair)}</span><br>
                      <span style="font-size:.76rem;color:#66766b;">{escape(codes)}</span>
                    </td>
                    <td style="padding:.52rem .45rem;border-bottom:1px solid #e5eee9;font-weight:900;color:{color};vertical-align:top;">{escape(answer)}</td>
                    <td style="padding:.52rem .45rem;border-bottom:1px solid #e5eee9;font-weight:900;color:{color};vertical-align:top;">{escape(result_text)}</td>
                  </tr>
                ''')

            st.markdown(
                f'''
                <div style="border:1px solid #d8e6dc;border-radius:18px;padding:1rem 1rem;background:#fff;box-shadow:0 8px 20px rgba(0,0,0,.035);max-width:100%;overflow:hidden;margin-bottom:1rem;">
                  <div style="font-size:1.02rem;font-weight:900;color:#10233f;margin-bottom:.2rem;">{escape(title)}</div>
                  <div style="color:#5a7060;font-size:.84rem;margin-bottom:.75rem;line-height:1.45;">{escape(subtitle)}</div>
                  <div style="width:100%;overflow:hidden;">
                    <table style="width:100%;max-width:100%;table-layout:fixed;border-collapse:collapse;background:#fff;border:1px solid #e5eee9;border-radius:12px;overflow:hidden;font-size:.80rem;">
                      <colgroup>
                        <col style="width:12%;"><col style="width:52%;"><col style="width:16%;"><col style="width:20%;">
                      </colgroup>
                      <thead>
        ...
        _render_table(
            "Step 1 — SP3 bottom-cause test",
            "First, Table A tests whether the lowest Part I line explains every line above it.",
            sp3_checks,
            "sp3",
        )

        sp3_passed = bool(sp3_checks) and all(x.get("accepted") is True for x in sp3_checks)
        if not sp3_passed:
            _render_table(
                "Step 2 — SP4/SP5 chain test",
                "Because SP3 did not pass, Table A checks the stacked chain line by line to see whether a valid run still reaches line (a).",
                adjacent,
                "adjacent",
            )

            if base_rule == "SP4":
                st.success(f"SP4 result: a valid causal run reaches line (a). Selected starting point: Part I ({selected_line}) — {selected_cause} ({selected_code}).")
            elif base_rule == "SP5":
    ...
    # -------------------------------------------------------------------------
    # Stage 3: Table A sequence check (SP3/SP4/SP5)
    # -------------------------------------------------------------------------
    elif stage == "rules":
        # Wider right panel so the Table A evidence table stays inside the page.
        ...
        with right_col:
            ...
            with btn_run:
                ...
                if st.button("Check Sequence", type="primary", disabled=not can_run_rules, use_container_width=True, key="run_table_ab_right_panel"):
                    with st.spinner("Checking sequence..."):
                        tabb_df = load_tabb_rules()
                        taba_df = load_taba_rules()
    ...
    # -------------------------------------------------------------------------
        ...
        with right_col:
            ...
            if st.session_state.get("agent4_result"):
                ...
                st.markdown("<div style='height:1rem'></div>", unsafe_allow_html=True)
                _render_quality_result_simple(st.session_state.get("agent4_result"), st.session_state.get("icd_results"))
