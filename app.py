...
# =============================================================================
# Controlled Claude Agent Orchestrator
# =============================================================================

AGENT_SYSTEM_CARD = """
Single-agent, tool-constrained death-certificate workflow.
Claude is allowed to: extract text, normalize medical phrases, select only from returned ICD candidates, and explain backend results.
Claude is NOT allowed to: invent ICD codes, invent causality, override SP1-SP8, or ignore ICD/TABB validation.
Final UCOD always comes from deterministic backend functions.
"""


def _line_dicts_to_causes(part1_payload: List[Dict[str, Any]], part2_payload: List[Dict[str, Any]]) -> Tuple[List[CauseLine], List[CauseLine]]:
    """Convert agent JSON payload into CauseLine objects, preserving form structure."""
    part1: List[CauseLine] = []
    for i, item in enumerate(part1_payload[:4]):
        line = str(item.get("line", "") or chr(ord("a") + i)).lower()
        if line not in {"a", "b", "c", "d"}:
            line = chr(ord("a") + i)
        part1.append(CauseLine(
            line=line,
            part="I",
            cause=str(item.get("cause", "") or "").strip(),
            interval=str(item.get("interval", "") or "").strip(),
            code=norm_code(item.get("code", "")),
            role="immediate" if i == 0 else ("underlying" if i == len(part1_payload[:4]) - 1 else "contributing"),
        ))
    # Pad Part I to keep UI assumptions stable.
    existing = {x.line for x in part1}
    for line in ["a", "b", "c", "d"]:
        if line not in existing:
            part1.append(CauseLine(line=line, part="I", role=""))
    part1 = sorted(part1, key=lambda x: ["a", "b", "c", "d"].index(x.line))

    part2: List[CauseLine] = []
    for i, item in enumerate(part2_payload[:3], start=1):
        part2.append(CauseLine(
            line=f"II-{i}",
            part="II",
            cause=str(item.get("cause", "") or "").strip(),
            interval=str(item.get("interval", "") or "").strip(),
    ...
    for i in range(len(part2) + 1, 4):
        part2.append(CauseLine(line=f"II-{i}", part="II", role="other"))
    return part1, part2


class DeathCertificateAgent:
    """A controlled one-agent orchestrator around deterministic tools.

    This class gives the app an explicit agent boundary:
    - Claude can extract and explain.
    - Python tools perform ICD lookup, TABB lookup, SP1-SP8, validation, and audit.
    """

    def __init__(self, icd_df: Optional[pd.DataFrame], tabb_df: Optional[pd.DataFrame], patient_sex: str):
        self.icd_df = icd_df
        self.tabb_df = tabb_df
        self.patient_sex = patient_sex

    def run_from_form(self, part1: List[CauseLine], part2: List[CauseLine], use_claude_explanation: bool = False) -> Dict[str, Any]:
        form_issues = form_validation(part1, part2)
        enriched_part1, enriched_part2, candidate_map, icd_issues = enrich_lines_with_icd(
            part1, part2, self.icd_df, self.patient_sex
        )
        sp_result, sp_issues, tabb_messages = apply_sp_rules(
            enriched_part1, enriched_part2, self.icd_df, self.tabb_df, self.patient_sex
        )
        issues = form_issues + icd_issues + sp_issues
        if use_claude_explanation:
            sp_result.explanation = claude_explain_result(sp_result, issues, enriched_part1 + enriched_part2)
        return self._package(enriched_part1, enriched_part2, candidate_map, sp_result, issues, tabb_messages, source="form")

    def run_from_narrative(self, narrative: str, use_claude_explanation: bool = True) -> Dict[str, Any]:
        extracted = claude_extract_narrative(narrative)
        if extracted.get("error"):
            empty = SPResult(applied_rule="Agent", explanation="Claude extraction failed.")
            issue = Issue("error", "Agent", "Narrative", extracted.get("error", "Extraction failed"), True)
            return self._package([], [], {}, empty, [issue], [], source="narrative")
        p1, p2 = _line_dicts_to_causes(extracted.get("part1", []), extracted.get("part2", []))
        result = self.run_from_form(p1, p2, use_claude_explanation=use_claude_explanation)
        result["agent_extraction"] = extracted
    ...
    def _package(
        ...
        blocking = any(i.blocking or i.severity == "error" for i in issues)
        warning_count = sum(1 for i in issues if i.severity == "warning")
        audit = {
            "agent_mode": "single_controlled_claude_agent",
            "agent_contract": AGENT_SYSTEM_CARD.strip(),
            "source": source,
            "part1": [asdict(x) for x in part1],
            "part2": [asdict(x) for x in part2],
            "sp_result": asdict(sp_result),
            "issues": [asdict(i) for i in issues],
            "tabb_messages": tabb_messages,
            "candidate_counts": {k: len(v) for k, v in candidate_map.items()},
            "ready_for_submit": not blocking,
            "review_required": sp_result.human_review_required or warning_count > 0 or blocking,
        }
        return {
            "part1": part1,
            "part2": part2,
...
def agent_status_message(result: Dict[str, Any]) -> str:
    sp = result.get("sp_result", SPResult())
    issues = result.get("issues", [])
    critical = sum(1 for i in issues if i.blocking or i.severity == "error")
    warnings = sum(1 for i in issues if i.severity == "warning")
    if critical:
        return f"Agent completed with {critical} critical issue(s). Submission should be blocked."
    if warnings or sp.human_review_required:
        return f"Agent completed with {warnings} warning(s). Coder review recommended."
    return "Agent completed. Certificate is ready for doctor/coder review."
...
with st.sidebar:
    ...
    use_controlled_agent = st.checkbox("Enable controlled Claude agent workflow", value=True)
    st.caption("Claude does not decide UCOD. It extracts/explains and calls deterministic backend logic.")
...
status_bits.append(pill("Controlled agent ON" if use_controlled_agent else "Agent OFF", "blue" if use_controlled_agent else "gray"))
...
with left:
    ...
    with ac2:
        run_agent_now = st.button("Run Controlled Agent Pass", use_container_width=True, disabled=not use_controlled_agent)
...
agent = DeathCertificateAgent(icd_df=icd_df, tabb_df=tabb_df, patient_sex=sex)

if use_controlled_agent and (live_validate or submit or run_agent_now):
    agent_result = agent.run_from_form(part1, part2, use_claude_explanation=use_claude_explain and (submit or run_agent_now))
    enriched_part1 = agent_result["part1"]
    enriched_part2 = agent_result["part2"]
    candidate_map = agent_result["candidate_map"]
    sp_result = agent_result["sp_result"]
    all_issues = agent_result["issues"]
    tabb_messages = agent_result["tabb_messages"]
    if run_agent_now:
        st.toast(agent_status_message(agent_result))
else:
    form_issues = form_validation(part1, part2)
    ...
    agent_result = agent._package(enriched_part1, enriched_part2, candidate_map, sp_result, all_issues, tabb_messages, source="manual_pipeline")

if use_claude_explain and (submit or st.session_state.get("last_explain", False)) and not use_controlled_agent:
...
with right:
    st.markdown("<div class='right-card'><h4>Agent Control</h4>", unsafe_allow_html=True)
    st.markdown(pill("One controlled Claude agent" if use_controlled_agent else "Manual deterministic pipeline", "blue" if use_controlled_agent else "gray"), unsafe_allow_html=True)
    st.caption("Agent extracts/explains only. ICD matching, TABB lookup, SP1–SP8, and UCOD validation stay deterministic.")
    if 'agent_result' in locals():
        st.markdown(f"<div class='info-box'>{html_escape(agent_status_message(agent_result))}</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
- **One controlled Claude agent** orchestrates extraction/explanation and backend tool calls. Do not use multiple agents for clinical decisions.
- Claude can extract narrative text, normalize conditions, and explain backend results.
