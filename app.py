"""Saudi MOH - Electronic Death Certificate SystemEnglish UIHybrid ICD-10 coding:

Claude extracts Part I / Part II causes + intervals

Excel / metadata / retrieval provide candidate rows

Claude selects ONLY from retrieved file candidates

Rule-based validation checks the final certificate

Source of truth for coding = loaded ICD file rows only.Claude is NOT allowed to invent ICD codes."""



import streamlit as stimport pandas as pdimport numpy as npimport reimport datetimeimport osimport pickleimport htmlimport jsonimport iofrom typing import List, Dict, Tuple, Optional
...
st.set_page_config(page_title="Death Certificate | Saudi MOH",layout="wide",initial_sidebar_state="expanded",)
...
try:API_KEY = st.secrets["ANTHROPIC_API_KEY"]except Exception:API_KEY = None

GDRIVE_EMBEDDINGS_ID = "1CxCGihYnqyaIc-F0IJwHNUTpLRHGmy8Y"GDRIVE_FAISS_ID      = "17F5rDFoT3iDbRKKiHCMiSB9ZrH0ecVTP"GDRIVE_METADATA_ID   = "1nUUdhivH1XIPGXkvWjrapxzuKfbM445B"GDRIVE_EXCEL_ID      = "1h54uBVeae8r6xC0MJI1-G3uRJwLc7K-y"

CACHE_DIR = os.path.join(os.path.expanduser("~"), ".icd10_hybrid_cache_v1")EMBED_MODEL_NAME = "pritamdeka/S-PubMedBert-MS-MARCO"CLAUDE_MODEL = "claude-sonnet-4-20250514"
...
def escape(x) -> str:return html.escape("" if x is None else str(x))

def sanitize_filename(s: str) -> str:s = re.sub(r"[^\w-.]+", "_", str(s).strip(), flags=re.UNICODE)return s[:120] if s else "certificate"

def normalize_text_basic(text: str) -> str:if not text:return ""text = str(text).strip().lower()text = text.replace("\n", " ")text = re.sub(r"\s+", " ", text)return text

def tokenize(text: str) -> List[str]:text = normalize_text_basic(text)return re.findall(r"[A-Za-z]+\d*.?\d*|[\u0600-\u06FF]+|\d+", text)

def specificity_score(code: str) -> int:if not code:return 0return len(str(code).strip().upper().replace(".", ""))

def is_gender_allowed(gender_restriction: str, sex_value: str) -> bool:gr = normalize_text_basic(gender_restriction)sx = normalize_text_basic(sex_value)if not gr or gr in {"", "none", "n/a", "nan", "unknown"}:return Trueif "female" in gr and "male" in sx:return Falseif "male" in gr and "female" in sx:return Falsereturn True

def acceptable_main_bool(x: str) -> Optional[bool]:t = normalize_text_basic(x)if t in {"acceptable", "yes", "true", "1"}:return Trueif t in {"not acceptable", "no", "false", "0"}:return Falsereturn None

def query_indicates_external_cause(query: str) -> bool:q = normalize_text_basic(query)triggers = ["accident", "injury", "collision", "fall", "burn", "poisoning", "vehicle","atv", "road traffic", "assault", "homicide", "suicide", "gunshot", "stab"]return any(t in q for t in triggers)

def diabetes_type_hint(query: str) -> Optional[str]:q = normalize_text_basic(query)if re.search(r"\b(type\s2|type\sii|non[-\s]?insulin[-\s]?dependent)\b", q):return "type2"if re.search(r"\b(type\s1|type\si|insulin[-\s]?dependent)\b", q):return "type1"return None
...
LAY_QUERY_EXPANSIONS = {"heart attack": ["acute myocardial infarction", "myocardial infarction", "coronary thrombosis"],"stroke": ["cerebral infarction", "cerebrovascular accident", "intracranial hemorrhage"],"kidney failure": ["renal failure", "chronic kidney disease", "acute kidney failure"],"high blood pressure": ["hypertension", "essential hypertension"],"diabetes": ["diabetes mellitus"],"type 2 diabetes": ["type 2 diabetes mellitus", "non insulin dependent diabetes mellitus"],"type ii diabetes": ["type 2 diabetes mellitus", "non insulin dependent diabetes mellitus"],"type 1 diabetes": ["type 1 diabetes mellitus", "insulin dependent diabetes mellitus"],"type i diabetes": ["type 1 diabetes mellitus", "insulin dependent diabetes mellitus"],"fluid in lungs": ["pulmonary edema", "acute pulmonary edema"],"lung infection": ["pneumonia", "lower respiratory infection"],"brain bleed": ["intracranial hemorrhage", "cerebral hemorrhage"],"blood clot in lung": ["pulmonary embolism"],"cancer spread": ["metastatic malignant neoplasm", "secondary malignant neoplasm"],"ards": ["acute respiratory distress syndrome"],"acute respiratory distress syndrome": ["ards"],"peritonitis": ["generalized peritonitis"],"diverticulitis": ["sigmoid diverticulitis", "diverticulitis of large intestine"],"septic shock": ["septic shock", "sepsis"],"obesity": ["obesity"],"metabolic syndrome": ["metabolic syndrome"],"vasculopathy": ["angiopathy", "vascular disease"],}

STOPWORDS = {"the", "and", "or", "with", "without", "due", "to", "secondary", "of", "in","acute", "chronic", "history", "known", "generalized", "severe", "mild","patient", "died", "from", "which", "developed", "resulted", "occurred","context", "background", "approximately", "about", "prior", "before", "after","complication", "condition", "lasting", "present", "nearly", "itself"}
...
def _gdrive_download(file_id: str, dest_path: str) -> None:import requests
...
EXPECTED_COLS = ["Id", "Code", "CodeFormatted", "ShortDesc", "LongDesc","HIPPA", "Deleted", "Classification", "AcceptableMain","GenderRestriction", "MatchSource", "MatchedFromCode", "Note"]

def _normalise_df(df: pd.DataFrame) -> pd.DataFrame:df = df.copy()
...
@st.cache_resource(show_spinner="Loading Excel from Google Drive...")def load_excel_from_drive_file(file_id: str) -> pd.DataFrame:os.makedirs(CACHE_DIR, exist_ok=True)xlsx_path = os.path.join(CACHE_DIR, "icd_source.xlsx")if not os.path.exists(xlsx_path):_gdrive_download(file_id, xlsx_path)df = pd.read_excel(xlsx_path)return _normalise_df(df)

@st.cache_resource(show_spinner="Loading metadata...")def load_metadata(metadata_id: str) -> pd.DataFrame:os.makedirs(CACHE_DIR, exist_ok=True)meta_path = os.path.join(CACHE_DIR, "metadata.pkl")if not os.path.exists(meta_path):_gdrive_download(metadata_id, meta_path)with open(meta_path, "rb") as f:df = pickle.load(f)return _normalise_df(df)

@st.cache_resource(show_spinner="Building BM25 index...")def build_bm25_index(df: pd.DataFrame):try:from rank_bm25 import BM25Okapicorpus = [tokenize(x) for x in df["EmbedText"].tolist()]return BM25Okapi(corpus)except Exception:return None

@st.cache_resource(show_spinner="Loading embedding model...")def get_embed_model():from sentence_transformers import SentenceTransformerreturn SentenceTransformer(EMBED_MODEL_NAME)

@st.cache_resource(show_spinner="Loading FAISS resources...")def load_faiss_resources(emb_id: str, faiss_id: str):os.makedirs(CACHE_DIR, exist_ok=True)emb_path = os.path.join(CACHE_DIR, "embeddings.npy")faiss_path = os.path.join(CACHE_DIR, "icd.index")
...
def expand_query(query: str) -> str:q = normalize_text_basic(query)expansions = []for key, vals in LAY_QUERY_EXPANSIONS.items():if key in q:expansions.extend(vals)return (query + " " + " ".join(expansions)).strip() if expansions else query

def row_to_dict(row, score: float = 0.0) -> Dict:return {"code": str(row.get("Code", "")),"code_formatted": str(row.get("CodeFormatted", "")),"short_desc": str(row.get("ShortDesc", "")),"long_desc": str(row.get("LongDesc", "")),"acceptable_main": str(row.get("AcceptableMain", "")),"gender_restriction": str(row.get("GenderRestriction", "")),"classification": str(row.get("Classification", "")),"note": str(row.get("Note", "")),"score": float(score),}

def exact_code_search(df: pd.DataFrame, query: str) -> List[Tuple[int, float]]:q = query.strip().upper().replace(" ", "")if re.fullmatch(r"[A-TV-Z][0-9]0-9?", q):mask = df["lookup_code"] == qhits = df.index[mask].tolist()return [(int(i), 1.0) for i in hits]return []

def semantic_search(df: pd.DataFrame, faiss_index, query: str, top_k: int = 50) -> List[Tuple[int, float]]:if faiss_index is None:return []try:model = get_embed_model()q_vec = model.encode([expand_query(query)], normalize_embeddings=True, convert_to_numpy=True).astype("float32")scores, indices = faiss_index.search(q_vec, top_k)return [(int(idx), float(score)) for score, idx in zip(scores[0], indices[0]) if idx != -1 and 0 <= idx < len(df)]except Exception:return []

def bm25_search(df: pd.DataFrame, bm25, query: str, top_k: int = 50) -> List[Tuple[int, float]]:q = expand_query(query)toks = tokenize(q)if not toks:return []
...
def reciprocal_rank_fusion(rank_lists: List[List[Tuple[int, float]]], k: int = 60) -> Dict[int, float]:fused = {}for lst in rank_lists:for rank, (idx, _) in enumerate(lst, start=1):fused[idx] = fused.get(idx, 0.0) + 1.0 / (k + rank)return fused

def candidate_adjustment_score(row: pd.Series, query: str, sex_value: str, role: str) -> Tuple[float, List[str]]:reasons = []score = 0.0
...
def search_icd_candidates(df_source: pd.DataFrame,faiss_index,bm25,query: str,sex_value: str,role: str,top_k: int = 12,) -> List[Dict]:exact_hits = exact_code_search(df_source, query)sem_hits = semantic_search(df_source, faiss_index, query, top_k=50)bm_hits = bm25_search(df_source, bm25, query, top_k=50)
...
def get_row_by_code(df: pd.DataFrame, code_str: str) -> Optional[pd.Series]:if df is None or not code_str:return Nonec = code_str.strip().upper().replace(" ", "")matches = df[df["lookup_code"] == c]if not matches.empty:return matches.iloc[0]return None
...
def _extract_text_from_claude_response(resp) -> str:parts = []for block in getattr(resp, "content", []):txt = getattr(block, "text", None)if txt:parts.append(txt)return "\n".join(parts).strip()

def _extract_json_candidate(text: str) -> str:if not text:return ""
...
def _try_parse_json_loose(text: str):if not text:raise json.JSONDecodeError("Empty response", "", 0)
...
def call_claude_json(api_key: str,system_prompt: str,user_prompt: str,max_tokens: int = 1200,fallback: Optional[dict] = None,) -> dict:client = anthropic.Anthropic(api_key=api_key)
...
def extract_causes_with_claude(api_key: str, narrative: str, patient_info: dict) -> dict:system_prompt = """You are a clinical death-certificate extraction assistant.

Task:Extract the death certificate structure from the doctor's narrative.
...
Preserve the direct causal chain for Part I in order:immediate cause -> due to -> due to -> underlying cause
...
Return JSON exactly in this schema:{"part1_chain": [{"cause": "string", "interval": "string"}],"part2_conditions": [{"cause": "string", "interval": "string"}]}"""user_prompt = f"""Patient information:Age: {patient_info.get("age_years", "Unknown")}Sex: {patient_info.get("sex", "Unknown")}Death type: {patient_info.get("death_type", "Unknown")}Chronic conditions: {", ".join(patient_info.get("chronic_conditions", []))}

Narrative:{narrative}"""return call_claude_json(api_key,system_prompt,user_prompt,max_tokens=900,fallback={"part1_chain": [],"part2_conditions": [],},)

def select_code_from_candidates_with_claude(api_key: str,cause_text: str,role: str,interval: str,sex_value: str,age_years: int,candidates: list,) -> dict:if not candidates:return {"selected_code": "","reason": "No retrieved file candidates available.","manual_review": True,"acceptable_main": "Unknown",}
...
You MUST choose only from the candidate rows provided.Do NOT invent codes.Do NOT use outside knowledge to create a new code.If none of the candidates are adequate, return manual_review=true and selected_code="".

Return JSON exactly:{"selected_code": "string","reason": "string","manual_review": true,"acceptable_main": "string"}"""user_prompt = f"""Cause text: {cause_text}Role: {role}Interval: {interval}Patient sex: {sex_value}Patient age: {age_years}

Candidate ICD rows:{json.dumps(slim_candidates, ensure_ascii=False, indent=2)}"""
...
def is_r_chapter(code: str) -> bool:return (code or "").strip().upper().startswith("R")

def _excel_text_flags(item: Dict) -> str:return normalize_text_basic(" ".join([item.get("classification", ""),item.get("acceptable_main", ""),item.get("note", ""),item.get("short_desc", ""),item.get("long_desc", ""),]))

def is_excel_ill_defined(item: Dict) -> bool:txt = _excel_text_flags(item)code = str(item.get("code_formatted", "")).upper()return ("ill-defined" in txt or "ill defined" in txt or "illdefined" in txt or code.startswith("R"))

def is_excel_unlikely_to_cause_death(item: Dict) -> bool:txt = _excel_text_flags(item)return "unlikely" in txt or "trivial" in txt or "not likely" in txt

def has_multiple_causes_in_one_line(cause_text: str) -> bool:# UI/form rule: one disease/condition per line. This is not ICD coding; it prevents input ambiguity.c = normalize_text_basic(cause_text)if not c:return Falsemarkers = [";", "/", " plus ", " along with "]if any(m in c for m in markers):return True# Commas often indicate more than one condition.if "," in c:return True# "and" is suspicious unless it is part of a single ICD phrase returned by Excel later.if re.search(r"\b(and|with)\b", c):return Truereturn False

def clean_cause_input(cause_text: str) -> str:"""Clean doctor-entered cause text before validation/retrieval."""c = str(cause_text or "").strip()c = re.sub(r"^(?[a-dA-D])?[.:-\s]+", "", c).strip()c = re.sub(r"^(part\si+\s)?(?[a-dA-D])?[.:-\s]+", "", c, flags=re.I).strip()c = re.sub(r"^(ii[-\s]\d+|part\sii[-\s]\d)[.:-\s]+", "", c, flags=re.I).strip()c = re.sub(r"\s+", " ", c)return c

def looks_like_non_medical_sentence(cause_text: str) -> bool:"""Detect narrative text that should not be placed in one COD line."""c = normalize_text_basic(cause_text)if not c:return Falsebad_phrases = ["patient died", "passed away", "became sick", "very sick", "peacefully","found dead", "was admitted", "was brought", "condition deteriorated","after being", "complained of", "because of unknown", "unknown reason",]if any(p in c for p in bad_phrases):return True# Long sentence-like entries are suspicious for a structured cause line.token_count = len(tokenize(c))if token_count >= 10 and any(w in c for w in [" patient ", " died ", " admitted ", " hospital ", " after "]):return Truereturn False

def pre_validate_structured_cod(part1_chain: List[Dict], part2_conditions: List[Dict]) -> Dict:"""Pre-coding validation. Blocks retrieval only for form-structure errors."""issues = []cleaned_part1, cleaned_part2 = [], []
...
def doctor_issue_card(issue: Dict) -> None:sev = issue.get("severity", "warning")msg = f"{issue.get('line','')}: {issue.get('message','')}"if sev == "error":st.error(msg)else:st.warning(msg)

def compact_candidate_label(c: Dict) -> str:code = c.get("code_formatted", "")desc = c.get("short_desc", "") or c.get("long_desc", "")return f"{code} — {desc}" if code else desc

def strict_quality_from_results(coded_causes: List[Dict], validation: Dict, pre_issues: Optional[List[Dict]] = None) -> str:"""Final quality: missing codes / blocking validation / sequence flags force Needs Review."""issues = list(validation.get("coding_issues", []) or [])if pre_issues and any(i.get("severity") == "error" for i in pre_issues):return "Needs Review"if not coded_causes:return "Needs Review"if any(not x.get("code_formatted") for x in coded_causes):return "Needs Review"if any(str(x.get("selection_status", "")) == "manual_review" for x in coded_causes):return "Needs Review"if issues:return "Needs Review" if len(issues) > 0 else "Good"return "Excellent"

def causal_sequence_check_with_claude(api_key: str, part1_items: List[Dict]) -> Dict:"""AI-assisted sequence plausibility check; does not invent ICD codes."""filled = [{"line": x.get("line", ""), "cause": x.get("cause", ""), "code": x.get("code_formatted", "")}for x in part1_items if str(x.get("cause", "")).strip()]if len(filled) <= 1:return {"valid": True, "warnings": [], "summary": "Single Part I cause; no causal chain to compare."}system_prompt = """You are a death-certificate sequence reviewer.Assess only whether each lower Part I condition can medically give rise to the condition immediately above it.Do not add new diseases. Do not invent ICD codes. Return only valid JSON.Schema:{"valid": true, "warnings": ["string"], "summary": "string"}"""user_prompt = "Part I chain, immediate to underlying:\n" + json.dumps(filled, ensure_ascii=False, indent=2)try:out = call_claude_json(api_key, system_prompt, user_prompt, max_tokens=500, fallback={"valid": True, "warnings": [], "summary": "Sequence check unavailable."})if not isinstance(out, dict):return {"valid": True, "warnings": [], "summary": "Sequence check returned an unexpected format."}return {"valid": bool(out.get("valid", True)),"warnings": list(out.get("warnings", []) or []),"summary": str(out.get("summary", "") or ""),}except Exception as e:return {"valid": True, "warnings": [], "summary": f"Sequence check failed: {type(e).name}: {e}"}



def normalize_cause_key(cause_text: str) -> str:"""Normalize cause text for duplicate detection."""c = normalize_text_basic(clean_cause_input(cause_text))c = re.sub(r"[^a-z0-9\u0600-\u06FF]+", " ", c)c = re.sub(r"\s+", " ", c).strip()return c

def validate_interval_text(interval_text: str, line_label: str) -> List[Dict]:"""Doctor-facing validation for approximate interval fields."""t = str(interval_text or "").strip().lower()if not t:return [{"severity": "warning","line": line_label,"type": "missing_interval","message": "Add an approximate interval, such as 2 days, 3 hours, 6 months, or unknown.","blocking": False,}]if t in {"unknown", "unk", "not known", "n/a", "na", "-", "—"}:return []# A number alone, e.g. 2 or 7, is ambiguous.if re.fullmatch(r"\d+(?:.\d+)?", t):return [{"severity": "warning","line": line_label,"type": "ambiguous_interval","message": "Interval needs a time unit, e.g., 2 days, 4 hours, or 7 years.","blocking": False,}]units = r"(minute|minutes|min|hour|hours|hr|hrs|day|days|week|weeks|month|months|year|years|yr|yrs)"if re.search(r"\b\d+(?:.\d+)?\s*" + units + r"\b", t):return []return [{"severity": "warning","line": line_label,"type": "unclear_interval","message": "Interval format is unclear. Use a simple duration such as 2 days or write unknown.","blocking": False,}]

def validate_cause_line_from_excel(cause_text: str,line_label: str,role: str,df_source: pd.DataFrame,faiss_index,bm25,sex_value: str,age_years: int = 0,top_k: int = 5,) -> Tuple[List[Dict], List[Dict]]:issues = []cause = str(cause_text or "").strip()
...
def determine_starting_point_from_structured_part1(part1_chain: List[Dict]) -> Dict:filled = [x for x in part1_chain if str(x.get("cause", "")).strip()]if not filled:return {"line": "", "cause": "", "interval": "—", "rule": "No Part I cause entered."}if len(filled) == 1:out = filled[0].copy()out["rule"] = "SP1/SP2: single completed Part I line."return outout = filled[-1].copy()out["rule"] = "Structured WHO form: lowest completed Part I line is the candidate starting point/underlying cause, subject to Excel validation and sequence review."return out

def validate_certificate(coded_causes: List[Dict], sex_value: str) -> Dict:issues = []
...
def code_extracted_causes_with_claude(api_key: str,extracted: dict,df_source: pd.DataFrame,faiss_index,bm25,patient_info: dict,) -> dict:coded_causes = []
...
def code_causes_hybrid_with_claude(api_key: str,narrative: str,df_source: pd.DataFrame,faiss_index,bm25,patient_info: dict,) -> dict:extracted = extract_causes_with_claude(api_key, narrative, patient_info)coded_causes = []
...
def refresh_code_from_manual_edit(df_source: pd.DataFrame, item: Dict, new_code: str, sex_value: str) -> Dict:row = get_row_by_code(df_source, new_code)updated = item.copy()
...
def generate_certificate_pdf(fd: dict,coded_causes: List[Dict],validation: dict,hospital_name: str,hospital_city: str,doctor_name: str,) -> bytes:"""Generate a professional PDF death certificate using reportlab."""from reportlab.lib.pagesizes import A4from reportlab.lib import colorsfrom reportlab.lib.units import mmfrom reportlab.platypus import (SimpleDocTemplate, Paragraph, Spacer, Table, TableStyle,HRFlowable, KeepTogether)from reportlab.lib.styles import getSampleStyleSheet, ParagraphStylefrom reportlab.lib.enums import TA_CENTER, TA_LEFT, TA_RIGHT
...
with st.sidebar:st.markdown("### Hospital Information")hospital_name = st.text_input("Hospital Name", value="King Fahad Specialist Hospital")hospital_city = st.text_input("City", value="Riyadh")doctor_name   = st.text_input("Certifying Physician", value="")
...
defaults = {"page": 1,"form_data": {},"icd_results": None,"df_source": None,"df_metadata": None,"faiss_index": None,"bm25_index": None,}for k, v in defaults.items():if k not in st.session_state:st.session_state[k] = v
...
if st.session_state["df_source"] is None:df_excel = Noneload_errors = []
...
def render_steps(current: int):labels = ["Basic Information","Medical History","Cause of Death","Review & Coding","Final Certificate",]html_s = ''for i, lbl in enumerate(labels, 1):cls = "step active" if i == current else ("step done" if i < current else "step")html_s += f'{i}. {escape(lbl)}'html_s += ""st.markdown(html_s, unsafe_allow_html=True)

def inline_note(message: str, level: str = "warning") -> None:icon = {"error": "✕", "warning": "⚠", "ok": "✓"}.get(level, "•")css = {"error": "inline-error", "warning": "inline-warning", "ok": "inline-ok"}.get(level, "inline-warning")st.markdown(f'{icon} {escape(message)}', unsafe_allow_html=True)

def issue_line_key(issue: Dict) -> str:line = str(issue.get("line", "")).lower()m = re.search(r"part\si\s(([a-d]))", line)if m:return f"part1_{m.group(1)}"m = re.search(r"part\sii\s(?([0-9]+))?", line)if m:return f"part2_{m.group(1)}"m = re.search(r"ii[-\s]*([0-9]+)", line)if m:return f"part2_{m.group(1)}"return "global"

def group_issues_by_field(issues: List[Dict]) -> Dict[str, List[Dict]]:out = {}for issue in issues:out.setdefault(issue_line_key(issue), []).append(issue)return out

def parse_interval_to_hours(interval_text: str) -> Optional[float]:t = normalize_text_basic(interval_text)if not t or t in {"unknown", "unk", "not known", "n/a", "na", "-", "—"}:return Nonem = re.search(r"(\d+(?:.\d+)?)\s*(minute|minutes|min|hour|hours|hr|hrs|day|days|week|weeks|month|months|year|years|yr|yrs)", t)if not m:return Noneval = float(m.group(1)); unit = m.group(2)if unit.startswith("min"): return val / 60.0if unit in {"hour", "hours", "hr", "hrs"}: return valif unit.startswith("day"): return val * 24if unit.startswith("week"): return val * 24 * 7if unit.startswith("month"): return val * 24 * 30if unit in {"year", "years", "yr", "yrs"}: return val * 24 * 365return None

def add_cross_field_cod_issues(part1_chain: List[Dict], part2_conditions: List[Dict]) -> List[Dict]:issues = []part1_keys = {normalize_cause_key(x.get("cause", "")): x for x in part1_chain if normalize_cause_key(x.get("cause", ""))}for x in part2_conditions:key = normalize_cause_key(x.get("cause", ""))if key and key in part1_keys:issues.append({"severity": "warning", "line": str(x.get("line", "Part II")), "type": "part2_repeats_part1", "message": "This condition is already in Part I. Part II should include only conditions that contributed but were not in the direct causal chain.", "blocking": False})cleaned = [x for x in part1_chain if x.get("cause")]for upper, lower in zip(cleaned, cleaned[1:]):h_upper = parse_interval_to_hours(upper.get("interval", ""))h_lower = parse_interval_to_hours(lower.get("interval", ""))if h_upper is not None and h_lower is not None and h_lower < h_upper:issues.append({"severity": "warning", "line": f"Part I ({lower.get('line','')})", "type": "interval_order", "message": "The cause below usually starts before the condition above. Check whether this interval should be longer or equal.", "blocking": False})return issues

def live_sequence_screen(part1_chain: List[Dict]) -> Dict:"""Lightweight live screen for obvious causal-sequence problems.
...
def decide_sp_rule_simple(part1_chain: List[Dict], blocking: bool, sequence_screen: Optional[Dict] = None) -> Dict:filled = [x for x in part1_chain if str(x.get("cause", "")).strip()]sequence_screen = sequence_screen or live_sequence_screen(filled)
...
def _certificate_conditions(part1_chain: List[Dict], part2_conditions: List[Dict]) -> List[Dict]:"""Return all non-empty certificate conditions in certificate order."""rows = []for x in part1_chain or []:cause = clean_cause_input(x.get("cause", ""))if cause:rows.append({"section": "Part I", "line": str(x.get("line", "")).lower(), "cause": cause})for x in part2_conditions or []:cause = clean_cause_input(x.get("cause", ""))if cause:rows.append({"section": "Part II", "line": str(x.get("line", "")), "cause": cause})return rows

def _coded_by_line(coded_causes: List[Dict]) -> Dict[str, Dict]:out = {}for item in coded_causes or []:line = str(item.get("line", "")).lower()if line:out[line] = itemreturn out

def _coded_by_cause(coded_causes: List[Dict]) -> Dict[str, Dict]:out = {}for item in coded_causes or []:key = normalize_cause_key(item.get("cause", ""))if key:out[key] = itemreturn out

def selected_code_for_sp(sp_review: Dict, coded_causes: List[Dict]) -> str:"""Find selected ICD code from the selected SP line/cause."""by_line = _coded_by_line(coded_causes)line = str(sp_review.get("selected_line", "")).lower()if line in by_line:return by_line[line].get("code_formatted", "")by_cause = _coded_by_cause(coded_causes)key = normalize_cause_key(sp_review.get("selected_cause", ""))if key in by_cause:return by_cause[key].get("code_formatted", "")return ""

def apply_sp_result_to_validation(validation: Dict, sp_review: Dict, coded_causes: List[Dict]) -> Dict:"""Preserve the SP-selected starting point independently from ICD coding.
...
def condition_flag_summary(item: Dict) -> Dict:"""Summarize Excel/metadata acceptability flags for SP7/SP8."""acc = acceptable_main_bool(item.get("acceptable_main", ""))return {"line": str(item.get("line", "")),"cause": str(item.get("cause", "")),"code": str(item.get("code_formatted", "")),"acceptable_main": acc,"ill_defined": is_excel_ill_defined(item),"unlikely_to_cause_death": is_excel_unlikely_to_cause_death(item),"gender_restriction": str(item.get("gender_restriction", "")),"note": str(item.get("note", "")),}

def deterministic_sp_fallback(part1_chain: List[Dict], part2_conditions: List[Dict]) -> Dict:"""Safe fallback if LLM sequence review is unavailable."""conditions = _certificate_conditions(part1_chain, part2_conditions)part1 = [x for x in conditions if x["section"] == "Part I"]if len(conditions) == 1:x = conditions[0]return {"sp_rule": "SP1","selected_line": x["line"],"selected_cause": x["cause"],"full_sequence_valid": None,"partial_sequence_valid": None,"causal_links": [],"warnings": [],"needs_manual_review": False,"explanation": "Only one condition is reported on the certificate.",}if len(part1) == 1:x = part1[0]return {"sp_rule": "SP2","selected_line": x["line"],"selected_cause": x["cause"],"full_sequence_valid": None,"partial_sequence_valid": None,"causal_links": [],"warnings": [],"needs_manual_review": False,"explanation": "Only one line is used in Part I.",}if part1:return {"sp_rule": "REVIEW","selected_line": "","selected_cause": "","full_sequence_valid": False,"partial_sequence_valid": False,"causal_links": [],"warnings": ["Medical sequence review is required before SP3-SP8 can be applied."],"needs_manual_review": True,"explanation": "Multiple Part I lines are present, so causal sequence review is required.",}return {"sp_rule": "REVIEW","selected_line": "","selected_cause": "","full_sequence_valid": False,"partial_sequence_valid": False,"causal_links": [],"warnings": ["No Part I condition is available."],"needs_manual_review": True,"explanation": "Part I is empty.",}

def llm_sp1_sp8_review(api_key: str, part1_chain: List[Dict], part2_conditions: List[Dict], coded_causes: List[Dict]) -> Dict:"""Apply simplified WHO-inspired SP1-SP8 logic.The LLM judges medical causality only. It must select only from written certificate causes.Excel/metadata flags are passed for SP7/SP8 refinement."""conditions = _certificate_conditions(part1_chain, part2_conditions)part1 = [x for x in conditions if x["section"] == "Part I"]
...
Task:Apply simplified WHO-inspired SP1-SP8 logic to identify the Starting Point from the certificate.The Starting Point becomes the basis for selecting the Underlying Cause of Death (UCOD).
...
SP logic:SP1: If only one condition is reported anywhere, select it.SP2: If only one Part I line is used, select that line.SP3: If multiple Part I lines are used and the lowest completed Part I line explains all entries above, select the lowest line.SP4: If the lowest line does not explain all entries above, but a valid partial sequence reaches the terminal condition on line (a), select the origin of that valid partial sequence.SP5: If no causal relationship can be established in Part I, select the first-mentioned Part I condition.SP6: If an obvious better underlying cause is present elsewhere on the certificate, select/suggest it.SP7: If the selected cause is ill-defined/vague, select a more specific condition from the certificate if available.SP8: If the selected cause is trivial or unlikely to cause death, select a more appropriate condition from the certificate if available.

Return JSON exactly:{"sp_rule": "SP3|SP4|SP5|SP6|SP7|SP8|REVIEW","selected_line": "a|b|c|d|II-1|II-2|II-3|","selected_cause": "string","full_sequence_valid": true,"partial_sequence_valid": true,"causal_links": [{"from_lower_line": "b", "to_upper_line": "a", "valid": true, "reason": "short reason"}],"warnings": ["string"],"needs_manual_review": false,"explanation": "short explanation"}"""user_payload = {"part1_chain_immediate_to_underlying": part1_chain,"part2_conditions": part2_conditions,"excel_metadata_flags_for_certificate_causes": coded_flags,"instruction": "Apply SP1-SP8. Select only a cause already written in this payload. Do not invent conditions or ICD codes.",}
...
def refine_sp7_sp8_with_excel(sp_review: Dict, coded_causes: List[Dict]) -> Dict:"""Use Excel/metadata flags to refine selected starting point for SP7/SP8."""if not coded_causes:return sp_review
...
def apply_sp_engine(api_key: str, concepts: Dict, coded_causes: List[Dict]) -> Dict:part1_chain = concepts.get("part1_chain", []) or []part2_conditions = concepts.get("part2_conditions", []) or []sp = llm_sp1_sp8_review(api_key, part1_chain, part2_conditions, coded_causes)sp = refine_sp7_sp8_with_excel(sp, coded_causes)sp["selected_code"] = selected_code_for_sp(sp, coded_causes)return sp

def quality_from_sp_and_validation(coded_causes: List[Dict], validation: Dict, sp_review: Dict) -> str:if not coded_causes:return "Needs Review"if any(not x.get("code_formatted") for x in coded_causes):return "Needs Review"if any(str(x.get("selection_status", "")) == "manual_review" for x in coded_causes):return "Needs Review"if sp_review.get("needs_manual_review") or sp_review.get("sp_rule") in {"REVIEW", "SP5", "SP7", "SP8"}:return "Needs Review"if validation.get("coding_issues"):return "Needs Review"return "Excellent"
...
if st.session_state.page == 1:render_steps(1)
...
elif st.session_state.page == 2:render_steps(2)fd = st.session_state.form_data
...
elif st.session_state.page == 3:render_steps(3)fd = st.session_state.form_datadf_source = st.session_state.df_sourcefaiss_index = st.session_state.faiss_indexbm25 = st.session_state.bm25_index
...
elif st.session_state.page == 4:render_steps(4)fd = st.session_state.form_datadf_source = st.session_state.df_sourcefaiss_index = st.session_state.faiss_indexbm25 = st.session_state.bm25_index
...
elif st.session_state.page == 5:render_steps(5)
